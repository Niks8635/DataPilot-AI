...
export default function ProfilePage() {
  ...
  const handleClearAllCustomDatasets = async () => {
    if (!confirm("Are you sure you want to delete all your uploaded datasets? Demo datasets will remain intact.")) {
      return;
    }
    try {
      const customOnes = profile?.datasets.filter((d) => !d.id.startsWith("demo-ds-")) || [];
      for (const ds of customOnes) {
        await api.datasets.delete(ds.id);
      }
      await loadProfile();
    } catch (err: any) {
      alert(`Error clearing datasets: ${err.message}`);
    }
  };
                        ...
                        {profile.datasets.map((ds) => (
                          <tr key={ds.id} className="hover:bg-white/[0.02] transition group">
                            ...
                            <td className="px-5 py-3.5 text-right space-x-2">
                              ...
                              {!ds.id.startsWith("demo-ds-") && (
                                <button
                                  onClick={() => handleDeleteDataset(ds.id, ds.name)}
                                  className="px-2.5 py-1 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 border border-rose-500/25 text-[11px] font-medium transition inline-flex items-center gap-1 active:scale-95"
